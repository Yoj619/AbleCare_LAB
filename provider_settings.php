 ...
 * - Load provider profile from DB: SELECT * FROM providers WHERE id = $_SESSION['provider_id']
 * - Save profile changes: UPDATE providers SET full_name=?, specialty=?, license_number=?, ... WHERE id=?
 * - Update password: verify current password, then UPDATE providers SET password_hash=? WHERE id=?
 * - Use password_hash() / password_verify() for secure password handling
 */
...
$provider = [
    ...
    'license_number'  => 'PRC-PT-12345',
    'years_experience'=> '15',
    'contact_number'  => '0956-789-0123',
    'email'           => 'jose.reyes@nasugbuhospital.com',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($_POST['action'] === 'save_profile') {
             ...
             * $stmt = $pdo->prepare("UPDATE providers SET full_name=?, specialty=?, ... WHERE id=?");
             * $stmt->execute([$_POST['full_name'], $_POST['specialty'], ..., $_SESSION['provider_id']]);
             */
            $profileSuccess = true;
        }
        ...
        if ($_POST['action'] === 'update_password') {
            /*
             * TODO: Validate current password, then update.
             ...
             *     $stmt = $pdo->prepare("UPDATE providers SET password_hash=? WHERE id=?");
             *     $stmt->execute([$hash, $_SESSION['provider_id']]);
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <div class="settings-card">
                ...
                <form method="POST" action="provider_settings.php">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input
                                ...
                                class="form-input"
                                value="<?= htmlspecialchars($provider['years_experience']) ?>"
                                min="0"
                            />
                        </div>
                        <div class="form-group full">
                            <label class="form-label" for="affiliated_facility">Affiliated Facility <span class="required">*</span></label>
                            <input
                                type="text"
                                id="affiliated_facility"
                                name="affiliated_facility"
                                class="form-input"
                                value="<?= htmlspecialchars($provider['hospital']) ?>"
                                required
                            />
                        </div>
                        <div class="form-group">
...
<script>
    ...
    function resetProfileForm() {
        ...
        document.getElementById('years_experience').value   = '<?= addslashes($provider['years_experience']) ?>';
        document.getElementById('affiliated_facility').value = '<?= addslashes($provider['hospital']) ?>';
