...
// ── Report data (replace with DB/PDO queries when tables exist) ──

// Summary stat cards
$summary_stats = []; // TODO: replace with DB query (e.g. counts of alerts/caregivers/providers per period)

// Therapy records table rows
$therapy_records = []; // TODO: replace with DB query (SELECT ... FROM therapy_sessions) when table exists

// Activity log entries
$activity_log = []; // TODO: replace with DB query (SELECT ... FROM activity_log) when table exists
?>
<!DOCTYPE html>
<html lang="en">
<head>
              ...
              <div class="stat-card-header">
                ...
                <span class="stat-trend">
                  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <polyline points="23 6 13.5 15.5 8.5 10.5 1 18" fill="none" stroke="<?= htmlspecialchars($s['color']) ?>" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                </span>
              </div>
      ...
      <div class="tab-panel<?= $active_tab === 'therapy' ? ' active' : '' ?>" id="panel-therapy">
        ...
        <div class="therapy-filter-row">
          ...
          <div class="filter-field">
            ...
            <input type="date" class="filter-input" id="th_date">
          </div>
          <div class="filter-field">
        ...
        <table id="therapyTable">
          ...
          <tbody>
            <?php if (empty($therapy_records)): ?>
              <tr><td colspan="6">
                <div class="empty-state">
                  <?= icon('log-icon', $svg) ?>
                  <p>No therapy records available yet.</p>
                </div>
              </td></tr>
            <?php else: ?>
